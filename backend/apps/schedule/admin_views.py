...
def session_create(request):
    if request.method == 'POST':
        try:
            ...
            # auto-create feedback form if enabled
...
def session_edit(request, pk):
    ...
    if request.method == 'POST':
        try:
            ...
            sess.start_datetime= request.POST['start_datetime']
            sess.end_datetime  = request.POST['end_datetime']
...
def subsession_add(request, session_pk):
    ...
    try:
        ...
        sub  = ScheduleSubSession.objects.create(
            ...
            start_datetime=body.get('start_datetime') or None,
            end_datetime=body.get('end_datetime') or None,
...
def feedback_toggle(request, session_pk):
    ...
    try:
        ...
        return JsonResponse({'success': True, 'feedback_manual_open': sess.feedback_manual_open, 'feedback_enabled': sess.feedback_enabled, 'feedback_open': sess.feedback_open})
    except Exception as e:
        return JsonResponse({'success': False, 'error': str(e)}, status=400)
...
def _ensure_feedback_form(sess):
    """Create default feedback form + questions if none exist."""
