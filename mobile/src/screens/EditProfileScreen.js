...
import {
  ...
  StatusBar, KeyboardAvoidingView, Platform, StyleSheet, Alert, Modal,
} from 'react-native';
  ...
  return (
    <View style={st.fieldGroup}>
      ...
      <View style={st.fieldRow}>
        <Ionicons name="flask-outline" size={17} color={COLORS.textTer} style={{ marginRight: SPACE.sm }} />
        <TextInput
          style={st.fieldInput}
          value={text}
...
export default function EditProfileScreen({ user, tokens, onBack, onProfileUpdated }) {
  ...
  const pickImage = async () => {
    // Use a simple approach without expo-image-picker:
    // Launch via the native file input on web, or use a simple approach for mobile
    try {
      ...
      Alert.alert('Photo Upload', 'Photo picker is not available. You can update your photo from the web.');
    }
  };
      ...
      if (res.ok && data.success) {
        ...
        if (data.points_awarded) {
          setPointsModal({
            ...
            message: data.points_message || `+${data.points_awarded} points earned!`,
          });
        }

        // Update photo URI from server response
        if (data.user?.profile_photo_url) {
          setPhotoUri(data.user.profile_photo_url);
        }
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      <View style={{ flex: 1, backgroundColor: COLORS.bg }}>
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={st.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <Card style={st.section} shadow="sm">
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: SPACE.sm, marginBottom: SPACE.lg }}>
              {GENDERS.map(g => (
                <TouchableOpacity
                  ...
                  style={[st.genderChip, form.gender === g && st.genderChipActive]}
                  activeOpacity={0.7}
                >
            ...
            <Field label="Designation" icon="briefcase-outline" value={form.designation} onChange={v => set('designation', v)} placeholder="e.g. Associate Professor" />
            <Field label="Phone" icon="call-outline" value={form.phone} onChange={v => set('phone', v)} placeholder="Mobile number" keyboardType="phone-pad" />
          </Card>
          ...
          <Card style={st.section} shadow="sm">
            ...
            <Field label="Affiliation / Organisation" icon="business-outline" value={form.affiliation} onChange={v => set('affiliation', v)} placeholder="e.g. IIT Delhi" />
            <Field label="Bio" icon="document-text-outline" value={form.bio} onChange={v => set('bio', v)} placeholder="A short bio about yourself..." multiline />
          ...
          <Card style={st.section} shadow="sm">
            ...
            <Field label="LinkedIn Profile" icon="logo-linkedin" value={form.linkedin_url} onChange={v => set('linkedin_url', v)} placeholder="https://linkedin.com/in/..." />
            ...
            <View style={st.toggleRow}>
              ...
              <TouchableOpacity onPress={() => set('show_phone', !form.show_phone)} style={[st.toggle, form.show_phone && st.toggleOn]}>
            ...
            <View style={[st.toggleRow, { borderBottomWidth: 0 }]}>
              ...
              <TouchableOpacity onPress={() => set('show_linkedin', !form.show_linkedin)} style={[st.toggle, form.show_linkedin && st.toggleOn]}>
